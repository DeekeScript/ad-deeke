...
const Index = {
    intoHome() {
        let homeTag = Common.id(V.Index.intoHome[0]).text(V.Index.intoHome[1]).findOnce();
        if (!homeTag) {
        ...
        if (App.getAppVersionCode('com.ss.android.ugc.aweme') == 330901) {
            let refreshTag = Common.id(V.Index.refresh[0]).text(V.Index.refresh[1]).isVisibleToUser(true).findOne();
            if (refreshTag) {
                Common.click(refreshTag);
                Common.sleep(2000 + Math.random() * 1000);
            }
        }
        Log.log('点击home');
    ...
    intoMyMessage() {
        //先点击“我” 再点击“首页”
        Common.sleep(3000);
        let msgTag = Common.id(V.Index.intoMyMessage[0]).text(V.Index.intoMyMessage[1]).findOnce();
        if (!msgTag) {
    ...
    intoMyPage() {
        Common.sleep(3000);
        let meTag = Common.id(V.Index.intoMyPage[0]).text(V.Index.intoMyPage[1]).findOnce();
        if (!meTag) {
    ...
    intoMyLikeVideo() {
        ...
        let likeTag;
        while (sw-- > 0) {
            likeTag = UiSelector().id(V.Index.intoMyLikeVideo[0]).textContains(V.Index.intoMyLikeVideo[1]).isVisibleToUser(true).findOnce();
            if (likeTag) {
                Log.log(likeTag);
        ...
        let containerTag;
        while (sw-- > 0) {
            containerTag = Common.id(V.Index.intoMyLikeVideo[2]).isVisibleToUser(true).filter((v) => {
                return v && v.bounds() && v.bounds().top + v.bounds().height() < Device.height() - 300 && v.bounds().width() > 0 && v.bounds().height() > 0;
            }).findOnce();
    ...
    intoLocal() {
        Log.log('进入同城');
        let tjs = Common.id(V.Index.intoLocal[0]).find();
        if (!tjs || tjs.length === 0) {
        ...
        for (let i in tjs) {
            //镇级别的地址，获取同城失败
            if (tjs[i].text() && tjs[i].text().indexOf('镇') != -1) {
                tj2 = tjs[i];
            }
        ...
        console.log('找到了同城', tj.parent());
        tj.parent().click();
        Common.sleep(3500);

        let tag = Common.id(V.Index.intoLocal[1]).clickable(true).isVisibleToUser(true).findOnce();
        console.log('点击同城', tag);
        if (tag) {
            tag.click();
            Common.sleep(1000);
        }

        let left = (Math.random() / 2 + 0.5) * Device.width();
        Gesture.swipe(left, Device.height() * (Math.random() / 4 + 0.5), left - Math.random() * 50, Device.height() * (Math.random() / 4 + 0.25), 300);
        Common.sleep(1000);
    },

    //进入推荐
    intoRecommend() {
        let tj = Common.id(V.Index.intoRecommend[0]).textContains(V.Index.intoRecommend[1]).findOnce();
        let top = tj.bounds().top + tj.bounds().height() / 2 * (Math.random() + 1 / 3);
        let left = tj.bounds().left;
        Gesture.swipe(left, top, left - 300, top, 300);
        Common.sleep(1000);
        Common.click(Common.id(V.Index.intoRecommend[0]).textContains(V.Index.intoRecommend[1]).findOnce());
        Common.sleep(2000);
    },
    //获取消息数
    getMsgCount() {
        let msgTag = Common.id(V.Index.getMsgCount[0]).textMatches(V.Index.getMsgCount[1]).findOnce();
        if (msgTag) {
    ...
    //左滑顶部tab
    swipeTopTabLeft() {
        let topTag = Common.id(V.Index.top[0]).isVisibleToUser(true).findOne();
        if (topTag) {
            topTag.scrollBackward();
            Log.log('顶部滑动成功');
        } else {
            Log.log('顶部滑动失败');
        }
    ...
    intoGroupBuy() {
        let i = 3;
        while (i-- > 0) {
            let tj = Common.id(V.Index.intoRecommend[0]).textContains(V.Index.intoRecommend[2]).findOnce();
            if (tj) {
                Common.click(tj);
                Common.sleep(2000);
                return true;
    ...
    intoSearchPage() {
        Common.sleep(3000);
        let searchTag = Common.id(V.Index.intoSearchPage[0]).isVisibleToUser(true).findOneBy(5000);
        Log.log(searchTag);
        if (!searchTag) {
    ...
    inRecommend() {
        let tag = UiSelector().desc(V.Index.inRecommend[2]).isVisibleToUser(true).findOne();
        Log.log('是否在推荐页', tag);
        if (tag) {
            return true;
        }
