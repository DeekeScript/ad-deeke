...
const Index = {
    ...
    intoMyPage() {
        ...
        Log.log('点击me' + ":" + tag.bounds().left + "::" + tag.bounds().top);
        return res;
    },
    ...
    isCity(text) {
        for (let i in citys) {
            if (citys[i].indexOf(text) != -1 || citys[i].indexOf(text + '县') != -1 || citys[i].indexOf(text + '镇') != -1 || citys[i].indexOf(text + '区') != -1) {
                return true;
            }
        }
        return false;
    ...
    intoLocal(cityName) {
        Log.log('进入同城');
        let recommendTag = UiSelector().className('android.widget.TextView').descContains('推荐').findOne();
        let tjs = recommendTag.parent().children().find(UiSelector().className('android.widget.TextView').filter(v => {
            return !!v.desc();
        }));
        ...
        let tj2 = undefined;
        for (let i in tjs) {
            let cName = tjs[i].desc().replace('已选中，', '').replace('，按钮', '');
            //镇级别的地址，获取同城失败
            if (cName && cName.indexOf('镇') != -1) {
                tj2 = tjs[i];
            }
            if (!cName || !this.isCity(cName)) {
                continue;
            }
            tj = tjs[i];
        }
        ...
        console.log('找到了同城', tj);
        if (tj.desc().indexOf('已选中') == -1) {
            tj.click();
            Common.sleep(3000 + 2000 * Math.random());
        } else {
            if (!cityName) {
                Log.log('没有设置同城名称');
                Gesture.click(Device.width() / 2, Device.height() / 2);
                return false;
            }
        }

        if (tj.desc().indexOf(cityName) != -1) {
            Log.log('同城已选中', tj.desc(), cityName);
            Gesture.click(Device.width() / 2, Device.height() / 2);
            return false;
        }

        Log.log('cityName：' + cityName);
        if (cityName) {
            tj.click();
            ...
            iptTag.setText(cityName);
            Common.sleep(500 + 500 * Math.random());

            //县城一般是这种格式：黄梅 · 黄冈
            let cityTag = UiSelector().className('android.widget.TextView').textContains(cityName).isVisibleToUser(true).filter(v => {
                return v.parent().className() == 'android.widget.LinearLayout';
            }).findOne();
            Log.log('点击cityTag：', cityTag);
            Common.click(cityTag);
            Common.sleep(3500);
        }

        let tag = UiSelector().desc('关闭').clickable(true).isVisibleToUser(true).findOnce();
        ...
        Common.sleep(1000);
        return Gesture.click(Device.width() / 2, Device.height() / 2);
    },
     ...
     */
    inRecommend() {
        let tag = UiSelector().descContains("推荐").findOne();
        Log.log('是否在推荐页', tag);
        if (tag.desc().indexOf('已选中') != -1) {
            return true;
        }
        return false;
    }
