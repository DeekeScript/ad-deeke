...
let iDy = {
    ...
    refreshVideo(videoRules, isCity) {
        ...
        while (true) {
            ...
            if (this.nicknames.includes(vNickname) || this.accountFreGt(vNickname)) {
                DyCommon.log(vNickname + '：重复或频率超出');
                continue;
            }
        ...
        if (videoRules.toker_view_video_second > 0) {
            DyCommon.log('随机休眠' + (videoRules.toker_view_video_second / 2) + '-' + videoRules.toker_view_video_second + 's');
    ...
    commentDeal(videoData) {
        ...
        DyCommon.log("评论频率：" + this.configData.toker_comment_rate);
        ...
        System.setAccessibilityMode('!fast');//非快速模式  否则拿不到点赞信息
        while (rps-- > 0) {
            Log.log('获取评论列表-开始');
    ...
    run(type) {
        ...
        if (this.isCity) {
            this.cityName = machine.get('toker_city_name');
        }
    ...
    runTask() {
        ...
        DyIndex.intoHome();
        if (this.isCity) {
            DyIndex.intoLocal(this.cityName);
        }
