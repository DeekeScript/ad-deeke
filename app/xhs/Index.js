...
let Index = {
    intoCity() {
        let cityTags = new UiSelector().className('androidx.appcompat.app.ActionBar$Tab').filter(v => {
            return !!v.desc();
        }).isVisibleToUser(true).find();

        for (let i in cityTags) {
            if (this.isCity(cityTags[i].desc())) {
                Log.log('进入同城', cityTags[i]);
                if (cityTags[i].isSelected()) {
                    return true;
                }
